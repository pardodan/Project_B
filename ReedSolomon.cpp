...
void Poly::print()
{
	...
	if (this->n > 0)
	{
		cout << "[" << setw(3) << dec << (RS_WORD)this->coef[0];
	}
	for (int i = 1; i < this->n; i++)
	{
		cout << ", " << setw(3) << dec << (RS_WORD)this->coef[i];
	}
	if (this->n > 0)
	{
		cout << "]" << dec << endl;
...
void ReedSolomon::calcSyndromes(Poly* out, Poly* msg, int nsym)
{
	...
	print_syndrom(temp,nsym);
}

void ReedSolomon::print_syndrom(RS_WORD* temp,int nsym)
{
    cout << "syndrom [ " ;
...
//Our decode with two syndroms decode
bool ReedSolomon::decode_v2(RS_WORD* wholeOut, RS_WORD* out, RS_WORD* data,RS_WORD* error_data, int k, int nsym, vector<unsigned int>* erasePos, bool debug)
{
    Poly synd;
    Poly msg(k + nsym, data);
    Poly synd_error;
    Poly msg_error(k + nsym, error_data);
    if (erasePos)
    {
        if (erasePos->size() > nsym)
        {
    ...
    this->calcSyndromes(&synd, &msg, nsym);
    this->calcSyndromes(&synd_error, &msg_error, nsym);
    Poly_Add(&synd,&synd,&synd_error);
    if (!this->checkSyndromes(&synd))
    {
        if(debug) cout << "errors detected, locating" << endl;
        Poly fsynd, errLoc;
...
bool test(int bits, int k, int nsym, int ncorr, bool print, bool* unmatch, int erasures)
{
    ...
    RS_WORD *originalData_b = (RS_WORD*)malloc(sizeof(RS_WORD) * (k + nsym));

    for (int i = 0; i < k; i++)
    {
        originalData[i] = rand() % (1 << bits);
        originalData_b[i]=originalData[i];
    }
    Poly msg(k, originalData);
    Poly synd_a; //syndrome of encoded msg
    Poly a(k + nsym, originalData);
    Poly b (k + nsym, originalData_b);
    ReedSolomon rs(bits);
    Poly synd_rs; //syndrome of decoded msg
    rs.encode(a.coef, originalData, k, nsym);
    rs.encode(b.coef, originalData_b, k, nsym);
    if (print)
    {
        cout << "Original message: " << endl;
        msg.print();
        cout << endl << "Encoded message: " << endl;
        a.print();
        rs.calcSyndromes(&synd_a,&a,nsym);
        cout << endl;

        cout << endl << "b message: " << endl;
        b.print();
        cout << endl;
    }
    ...
    bool success = rs.decode_v2(a.coef, msg.coef, a.coef,b.coef, k, nsym, erasures ? &erasePos : nullptr, print);
...
bool run_reed_solomon(int bits, int k, int nsym, int ncorr, bool print, bool* unmatch, int erasures,unsigned long* read)
{
    RS_WORD *originalData = (RS_WORD*)malloc(sizeof(RS_WORD) * (k + nsym));
    RS_WORD *originalData_b = (RS_WORD*)malloc(sizeof(RS_WORD) * (k + nsym));
    cout<<read[8]<<endl;
