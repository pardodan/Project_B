...
int main()
{
    ...
    std::cout<<endl<<"printing data_in:"<<endl;
    for (int i: data_in) {
        std::cout << i << ' ';
    }
    ...
    std::cout<<endl<<"printing data_test:"<<endl;
    for (int i: data_test) {
        std::cout << i << ' ';
    }
    ...
    vector<vector<int>> syndrome_error = code.syndrome_BCH(data_test,a_table,H);

    cout << endl << "print the error syndrome:" <<endl;
    for (int i=0;i<(int)syndrome_error.size();i++){ 
        
        for (int j=0;j<(int)syndrome_error.at(0).size();j++) 
        cout << syndrome_error[i][j] << " ";
        cout << endl;
    }

    vector<vector<int>> syndrome_test = syndrome_error;
    for (int i=0;i<(int)syndrome.size();i++){ 
        
        for (int j=0;j<(int)syndrome.at(0).size();j++) 
            syndrome_test[i][j] =  (syndrome[i][j] ^ syndrome_error[i][j]); 
    }
    
    cout << endl << "print the syndrome_test:" <<endl;
    for (int i=0;i<(int)syndrome_test.size();i++){ 
        
        for (int j=0;j<(int)syndrome_test.at(0).size();j++) 
        cout << syndrome_test[i][j] << " ";
        cout << endl;
    }

    
    // vector<vector<int>> sigma_x = code.decode_BCH(a_table, syndrome_test);
    vector<vector<int>> sigma_x = code.decode_BCH_v2(a_table,data_test,syndrome,H);
    vector<int> error_loc = code.error_loc_BCH(a_table,sigma_x);
    vector<int> fix_error = code.repair_bit(data_test,error_loc);
    fix_error.shrink_to_fit();
    // vector<int> decoded (fix_error.cbegin()+n-k,fix_error.cend());
    vector<int> different = code.xor_poly_bin(data_in,fix_error);


    // check if the encode vector and thr decode vector are different
    std::cout<<endl<<"checking decode: "<<endl;
    for (int i: different){
    ...
    // vector <int> data = code.random_bin_data(k);
    // vector <int> encode = code.encode_BCH(data,g_x,n-k);
    
    // std::cout<<endl<<"printing encoded data"<<endl;

    // for (int i: encode) {
    //     std::cout << i << ' ';
    // }
   
    // // creating 2 errors in the code
    // encode[0] = (1^encode[0]);
    // encode[1] = (1^encode[1]);
    
    // // third error should break the code
    // // encode[2] = (1^encode[2]);

    // std::cout<<endl<<"printing errored data"<<endl;
    // for (int i: encode) {
    //     std::cout << i << ' ';
    // }

    // vector <int> encode_with_error = encode; 
    // vector<vector<int>> syndrome = code.syndrome_BCH(encode_with_error,a_table,H);
    // vector<vector<int>> sigma_x = code.decode_BCH(a_table, syndrome);
    // vector<int> error_loc = code.error_loc_BCH(a_table,sigma_x);
    // vector<int> fix_error = code.repair_bit(encode_with_error,error_loc);
    // fix_error.shrink_to_fit();
    // vector<int> decoded (fix_error.cbegin()+n-k,fix_error.cend());
    // vector<int> different = code.xor_poly_bin(data,decoded);
